...
def medal_card(rank: int, driver_name: str, score: float, wins: int, podiums: int, points: float, medal_color: str):
    """Display a top-three driver card with native Streamlit elements."""
    medal = "??" if rank == 1 else "??" if rank == 2 else "??"

    with st.container(border=True):
        st.markdown(f"### {medal} #{rank} {driver_name}")
